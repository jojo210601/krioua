    ...
    <main className="relative min-h-screen pt-40 pb-24">
      <div className="container">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          {[
            { icon: Phone, label: 'Numéro Suisse', val: SWISS_PHONE, href: `tel:${SWISS_PHONE.replace(/\D/g, '')}` },
            { icon: Phone, label: 'Numéro français', val: FRENCH_PHONE, href: `tel:${FRENCH_PHONE.replace(/\D/g, '')}` },
            { icon: MapPin, label: 'Adresse', val: ADDRESS },
            ...
            <Reveal key={i} delay={i * 0.05}>
              <a href={c.href || '#'} className="block glass rounded-[24px] p-6 hover:shadow-glassLg transition-all duration-500 h-full">
                ...
                <div className="text-sm font-medium text-inkDark break-words">{c.val}</div>
              </a>
            </Reveal>
          ))}
        </div>
              ...
              {sent ? (
                ...
                <form onSubmit={onSubmit} className="space-y-4">
                  <div className="grid sm:grid-cols-2 gap-4">
                    <label className="block"><span className="text-xs text-inkSoft mb-2 block">Nom</span><input required value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="w-full glass rounded-2xl px-5 py-4 outline-none focus:shadow-glassLg transition-all text-sm text-inkDark placeholder:text-inkSoft/60" placeholder="Votre nom" /></label>
                    <label className="block"><span className="text-xs text-inkSoft mb-2 block">Email</span><input required type="email" value={form.email} onChange={e => setForm({...form, email: e.target.value})} className="w-full glass rounded-2xl px-5 py-4 outline-none focus:shadow-glassLg transition-all text-sm text-inkDark placeholder:text-inkSoft/60" placeholder="vous@exemple.ch" /></label>
                  </div>
                  <label className="block"><span className="text-xs text-inkSoft mb-2 block">Téléphone (optionnel)</span><input type="tel" value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} className="w-full glass rounded-2xl px-5 py-4 outline-none focus:shadow-glassLg transition-all text-sm text-inkDark placeholder:text-inkSoft/60" placeholder="+41 XX XXX XX XX" /></label>
                  <label className="block"><span className="text-xs text-inkSoft mb-2 block">Message</span><textarea required rows={5} value={form.message} onChange={e => setForm({...form, message: e.target.value})} className="w-full glass rounded-2xl px-5 py-4 outline-none focus:shadow-glassLg transition-all text-sm text-inkDark placeholder:text-inkSoft/60 resize-none" placeholder="Dites-moi ce qui vous amène, votre disponibilité…" /></label>
                  <button type="submit" className="btn-pill btn-primary w-full justify-center text-[15px] py-4 mt-2">Envoyer <Send className="h-4 w-4" /></button>
                </form>
              )}
            </div>
          </Reveal>
          ...
          <div className="lg:col-span-5 space-y-4">
            <Reveal delay={0.1}>
              <div className="glass-strong rounded-[32px] p-8">
                ...
                <div className="rounded-2xl border border-inkDark/10 bg-cream/50 h-[280px] flex flex-col items-center justify-center text-center p-6">
                  <div className="h-12 w-12 rounded-full bg-terracotta/15 flex items-center justify-center mb-4"><Clock className="h-5 w-5 text-terracotta" /></div>
                  ...
                  <a href={SITE.calendlyUrl} target="_blank" rel="noopener noreferrer" className="btn-pill btn-primary text-[13px] py-2.5 px-5">Ouvrir Calendly <ArrowRight className="h-4 w-4" /></a>
                </div>
              </div>
            </Reveal>
            ...
            <Reveal delay={0.15}>
              <div className="glass-strong rounded-[32px] p-2 overflow-hidden">
                <div className="rounded-[26px] overflow-hidden aspect-[4/3]"><iframe src={SITE.mapEmbed} width="100%" height="100%" style={{ border: 0 }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" title="Localisation du cabinet à Genève" /></div>
              </div>
            </Reveal>
          </div>
        </div>
        ...
        <Reveal>
          <div className="glass-strong rounded-[40px] p-12 text-center relative overflow-hidden">
            ...
            <div className="relative">
              ...
              <a href={SITE.calendlyUrl} target="_blank" rel="noopener noreferrer" className="btn-pill btn-primary text-[16px] px-10 py-5">Prendre rendez-vous <ArrowRight className="h-5 w-5" /></a>
            </div>
          </div>
        </Reveal>
      </div>
