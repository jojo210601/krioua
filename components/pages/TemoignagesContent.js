  ...
  return (
    <main className="relative min-h-screen pt-40 pb-24">
      <div className="container">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 mb-16">{TESTIMONIALS.map((t, i) => <Reveal key={i} delay={i * 0.03}><div className="glass rounded-[28px] p-8 h-full flex flex-col hover:shadow-glassLg transition-all duration-500"><div className="flex items-center justify-between mb-5"><div className="flex items-center gap-1 text-terracotta">{[...Array(t.rating)].map((_, k) => <Star key={k} className="h-3.5 w-3.5 fill-current" />)}</div>{t.video && <div className="text-[10px] uppercase tracking-widest text-sage">Vidéo</div>}</div><p className="display text-xl text-inkDark leading-snug mb-6">« {t.text} »</p><div className="mt-auto flex items-center gap-3 pt-4 border-t border-inkDark/5"><div className="h-10 w-10 rounded-full bg-gradient-to-br from-sand to-brownLight" /><div><div className="text-sm font-medium text-inkDark">{t.name}</div><div className="text-xs text-inkSoft">{t.city}</div></div></div></div></Reveal>)}</div>
